...
Our release pipeline currently builds the final integrated Neo4J data product for consumption. We do not execute this as part of the default pipeline run but with a separate `-p kg_release` execution as we do not want to release every pipeline data output.
