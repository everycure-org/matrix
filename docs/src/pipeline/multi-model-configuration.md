...
1. Add model configuration in `pipelines/matrix/src/matrix/settings.py`, which is built with `matrix.settings.generate_dynamic_pipeline_mapping` and wrapped by `matrix.settings.disable_private_datasets`, which is built with `matrix.settings.generate_dynamic_pipeline_mapping` and wrapped by `matrix.settings.disable_private_datasets`:
   ```python
   {
