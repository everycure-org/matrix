...
- **Running one node locally with full cloud data:** `--from-env cloud` Enables reading data from specific environments as part of the `kedro run` command, enhancing flexibility during development and testing. This allows developers to test just their changes on full data locally, essentially "forking" off the cloud environment. [(Documentation)](../../../getting_started/first_steps/local-setup.md)
