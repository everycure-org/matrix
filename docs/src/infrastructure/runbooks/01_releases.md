...
There might be cases that someone is preparing a munal release and has pushed the branch `release/<INTENDED_VERSION>`, but no associated tag exists yet. The scheduled [GitHub Action](https://github.com/everycure-org/matrix/blob/main/.github/workflows/submit-kedro-pipeline.yml) increments the version based on latest remote tags. If it happens to bump the version to the intended one, the action will fail attempting to push the branch `release/<INTENDED_VERSION>` because it already exists in the remote git repository. The failure will trigger a slack notification. 
