...
$$
\text{max}_{\text{parameters}} \frac{1}{J} \sum_{j = 1}^{J} \mathbb{E}_{\text{folds}}\left( a \cdot \text{drug-entropy}@n_j + b \cdot \text{disease-entropy}@n_j + c \cdot \text{recall}@n_j + d \cdot \text{hit}@k_j \right)
$$
