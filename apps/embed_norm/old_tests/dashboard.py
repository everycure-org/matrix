...
def get_available_models_and_datasets():
    ...
    datasets = set()
    embedding_types = set()
    embedding_pattern = re.compile(r"^embeddings_(.+?)_(.+?)(_combinations)?_seed_\d+\.pkl$")
    for filename in os.listdir(cache_dir):
        match = embedding_pattern.match(filename)
        if match:
            category_name, model_name, comb_flag = match.groups()
            embedding_type = "Combination" if comb_flag == "_combinations" else "Standard"
            models.add(model_name)
            datasets.add(category_name)
            embedding_types.add(embedding_type)
    return sorted(models), sorted(datasets), sorted(embedding_types)


embedding_models, datasets, embedding_types = get_available_models_and_datasets()
...
def update_plot(
    ...
    for s_type, seed in [("positive", seed1), ("negative", seed2)]:
        embeddings_files[s_type] = os.path.join(
            cache_dir, f"embeddings_{dataset_name}_{embedding_method}{embedding_suffix}_seed_{seed}.pkl"
        )
        labels_files[s_type] = os.path.join(
            cache_dir, f"labels_{dataset_name}_{embedding_method}{embedding_suffix}_seed_{seed}.pkl"
        )
        similarities_files[s_type] = os.path.join(
            cache_dir, f"cosine_similarities_{dataset_name}_{embedding_method}{embedding_suffix}_seed_{seed}.pkl"
        )
    ...
    if embeddings_combined.shape[0] != len(labels_list) or embeddings_combined.shape[0] != len(sample_types):
        # logging.error("Mismatch in the number of embeddings and labels.")
        return {}
    ...
    fig.update_layout(
        title=f"Embeddings Visualization ({embedding_type} Embeddings)",
        hovermode="closest",
