"""
Python port of src/bin/primekg.rs using Click.
...
def build_nodes_cmd(drug_features: Path, disease_features: Path, nodes: Path, output: Path) -> None:
    ...
    main = _coalesce_duplicate_columns(main, keep=["node_index"])  # preserves first non-null among suffixed columns
    ...
    main = _coalesce_duplicate_columns(main, keep=["node_index"])  # again after join
        ...
        source_mondo = (
            ...
            .with_columns(
                [
                    ...
                    pl.when(pl.col(source) == pl.lit("MONDO_grouped")).then(pl.lit("MONDO")).otherwise(pl.col(source)).alias(source),
                ]
            )
            .explode(_id)
        )
...
def _coalesce_duplicate_columns(df: pl.DataFrame, keep: list[str]) -> pl.DataFrame:
