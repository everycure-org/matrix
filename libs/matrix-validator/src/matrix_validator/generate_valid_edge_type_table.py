...
def generate_valid_biolink_edge_types():
    ...
    for predicate in predicates:
        ...
        all_subject_categories = (
            [sv.get_uri(class_name) for class_name in sv.class_descendants(slot.domain)] if slot.domain else None
        )
        all_object_categories = (
            [sv.get_uri(class_name) for class_name in sv.class_descendants(slot.range)] if slot.range else None
        )
    ...
    for class_name in sv.class_descendants("association"):
        ...
        all_predicates = [
            sv.get_uri(predicate_name) for predicate_name in sv.slot_descendants(slot_usage["predicate"].subproperty_of)
        ]
    ...
    all_valid_triples_df = pd.merge(
        predicate_valid_triples_df,
        association_valid_triples_df,
        how="outer",
        on=["subject_category", "predicate", "object_category"],
    )

    output_path = f"src/matrix_validator/resources/{VALID_EDGE_TYPE_TABLE}"
