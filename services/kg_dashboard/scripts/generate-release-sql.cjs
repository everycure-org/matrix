...
/**
 * Mapping from globals.yml data source names to BigQuery upstream_data_source values
 * BigQuery may use different naming conventions (e.g., hyphens vs underscores)
 */
...
`;
  ...
  const unionClauses = kgNames.map(kg => {
    ...
    return `  SELECT '${bqName}' as knowledge_graph, '${getKGDisplayName(kg)}' as display_name, '${versions[kg]}' as version`;
  });
