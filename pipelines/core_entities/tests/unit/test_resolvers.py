...
def test_purple_book_url_defaults_to_previous_utc_month_and_year(monkeypatch) -> None:
    ...
    expected_month = last_month_utc.strftime("%B").lower()
    expected_year = str(last_month_utc.year)

    result = purple_book_url()

    assert f"/files/{expected_year}/" in result
...
def test_purple_book_url_falls_back_when_last_month_is_unavailable(monkeypatch) -> None:
    ...
    assert result == "https://purplebooksearch.fda.gov/files/2026/purplebook-search-january-data-download.csv"
...
def test_purple_book_url_fallback_adjusts_year_across_january_boundary(monkeypatch) -> None:
    ...
    assert result == "https://purplebooksearch.fda.gov/files/2025/purplebook-search-november-data-download.csv"
...
def test_purple_book_url_uses_environment_overrides(monkeypatch) -> None:
    ...
    assert result == "https://purplebooksearch.fda.gov/files/2024/purplebook-search-january-data-download.csv"
...
def test_purple_book_url_prefers_explicit_arguments_over_environment(monkeypatch) -> None:
    ...
    assert result == "https://purplebooksearch.fda.gov/files/2025/purplebook-search-march-data-download.csv"
