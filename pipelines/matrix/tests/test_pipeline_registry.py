...
def test_default_pipeline_composition(pipelines):
    ...
    assert (
        len(default_pipeline.nodes) == len(make_embeddings.nodes) + len(make_modelling.nodes)
...
def test_make_modelling_composition(pipelines):
    ...
    assert (
        len(make_modelling.nodes) == len(modelling.nodes) + len(matrix_generation.nodes) + len(evaluation.nodes)
...
def test_test_pipeline_composition(pipelines):
    ...
    assert test_pipeline.nodes == (fabricator + ingestion + make_embeddings + make_modelling + release).nodes
