...
def sample_rank_sets():
    ...
    return (
        pd.DataFrame({"id": [1, 2, 3, 4, 5], "rank": [12, 522, 33, 14, 1]}),
        pd.DataFrame({"id": [1, 2, 3, 5, 6], "rank": [1, 2, 3, 4, 5]}),
    )
...
def sample_hypergeom_sets():
    """Fixture that provides sample rank sets for testing."""
    return (
        pd.DataFrame({"id": [1, 2, 3, 4, 5, 6], "rank": [1, 2, 3, 4, 5, 6]}),  # Increased to 6 items
        pd.DataFrame({"id": [3, 4, 5, 7, 8, 9], "rank": [1, 2, 3, 4, 5, 6]}),  # Keep 5 items to ensure overlap
    )
...
def sample_commonality_matrices():
    ...
    return [pd.DataFrame({"id": [1, 2, 3]}), pd.DataFrame({"id": [1, 3, 5]})]
...
def test_hypergeom_at_n(sample_hypergeom_sets):
    ...
    result = hypergeom_evaluator.generate()(sample_hypergeom_sets, common_items=[3, 4, 5])

    result_same = hypergeom_evaluator.generate()(
        (sample_hypergeom_sets[0], sample_hypergeom_sets[0]), common_items=[3, 4, 5]
    )
...
def test_spearman_at_n(sample_rank_sets):
    ...
    result = spearman_evaluator.generate()(sample_rank_sets, common_items=[1, 2, 3, 4])

    # Second one is testing correlation
    result_same = spearman_evaluator.generate()((sample_rank_sets[0], sample_rank_sets[0]), common_items=[1, 2, 3, 4])
