...
def test_unify_nodes(spark, sample_nodes, sample_biolink_category_hierarchy):
    ...
    core_id_mapping = spark.createDataFrame(
        [], schema="normalized_id string, core_id string, core_name string, core_category string"
    )
...
def test_core_promotion(spark, sample_nodes, sample_biolink_category_hierarchy):
    ...
    core_mapping_data = [("CHEBI:119157", "CORE_DRUG_1", "CORE_NAME_1", "biolink:Drug")]
    core_id_mapping = spark.createDataFrame(
    ...
    # Check that core_catefory promotion worked
    promoted_category = [row.category for row in result.collect()]
    assert "biolink:Drug" in promoted_category  # biolink:Drug should be promoted
