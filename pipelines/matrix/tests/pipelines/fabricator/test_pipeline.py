...
def test_remove_overlap_with_overlap():
    ...
    drug_list = pd.DataFrame({"curie": ["DRUG:1", "DRUG:2", "COMMON:1"], "other_drug_col": [1, 2, 3]})
    disease_list = pd.DataFrame(
        {"category_class": ["DIS:A", "DIS:B", "COMMON:1"], "other_disease_col": ["a", "b", "c"]}
    )
    expected_drug_list = pd.DataFrame({"curie": ["DRUG:1", "DRUG:2"], "other_drug_col": [1, 2]})
    expected_disease_list = pd.DataFrame({"category_class": ["DIS:A", "DIS:B"], "other_disease_col": ["a", "b"]})
...
def test_remove_overlap_without_overlap():
    ...
    drug_list = pd.DataFrame({"curie": ["DRUG:1", "DRUG:2"], "other_drug_col": [1, 2]})
    disease_list = pd.DataFrame({"category_class": ["DIS:A", "DIS:B"], "other_disease_col": ["a", "b"]})
...
def test_remove_overlap_empty_inputs():
    ...
    empty_drug_list = pd.DataFrame(columns=["curie", "other_drug_col"])
    empty_disease_list = pd.DataFrame(columns=["category_class", "other_disease_col"])
