...
class LazyPolarsCacheDataset(LazyPolarsDataset):
    """Extends LazyPolarsDataset to be used for our inference cache.

    This dataset has the special property that it always appends to the existing data.
    We leverage partitioning for this and always add a "invocation_id" column to the
    whole dataset which we then partition on.
    """

    pass


class LazyPolarsDataset(AbstractDataset[pl.LazyFrame, PolarsFrame]):
    """``LazyPolarsDataset`` loads/saves data from/to a data file using an underlying filesystem (e.g.: local, S3, GCS).

    It uses Polars to handle
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """Creates a new instance of ``LazyPolarsDataset`` pointing to a concrete data file on a specific filesystem.
