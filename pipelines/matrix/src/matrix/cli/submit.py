...
# fmt: on
def submit(username, namespace, run_name):
...
def check_dependencies():
    ...
    if not active_account:
        ...
        run_subprocess("gcloud auth login")
    ...
    try:
        run_subprocess("kubectl get nodes", capture_output=True)
    ...
    # Verify kubectl
    try:
        run_subprocess("kubectl get ns", capture_output=True)
    except subprocess.CalledProcessError:
...
def build_push_docker(username: str):
    """Build and push Docker image."""
    click.echo("Building and pushing Docker image...")
    run_subprocess(f"make docker_push TAG={username}")


def build_argo_template(run_name, username, namespace):
    """Build Argo workflow template."""
    click.echo("Building Argo workflow template...")
    image_name = "us-central1-docker.pkg.dev/mtrx-hub-dev-3of/matrix-images/matrix"
    generate_argo_config(image_name, run_name, username, namespace)
...
def ensure_namespace(namespace):
    ...
    if result.returncode != 0:
        ...
        run_subprocess(f"kubectl create namespace {namespace}", check=True)


def apply_argo_template(namespace):
    """Apply the Argo workflow template."""
    click.echo("Applying Argo workflow template...")
    run_subprocess(
        "kubectl apply -f templates/argo-workflow-template.yml -n matrix",
        check=True,
    )


def submit_workflow(run_name, namespace):
    """Submit and monitor the Argo workflow."""
    click.echo("Submitting Argo workflow...")
    submit_cmd = (
        f"argo submit --name {run_name} -n {namespace} "
        f"--from wftmpl/matrix -p run_name={run_name} "
        f"-l submit-from-ui=false --entrypoint __default__ -o json"
    )
    result = run_subprocess(submit_cmd, capture_output=True)
    ...
    click.echo(f"Watching job: {job_name}")
    run_subprocess(f"argo watch -n {namespace} {job_name}")
...
def get_run_name(run_name: Optional[str]) -> str:
    ...
    console.print(f"[blue]Branch sanitized:[/blue] {branch_sanitized}")
    return branch_sanitized
