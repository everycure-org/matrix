...
#             w[pos_mask] = self._apply_budget(X.loc[pos_mask, [self.head_col]],
#                                             w[pos_mask], self.head_col, self.class_budget_pos)
#         if self.enforce_budget and neg_mask.any():
#             w[neg_mask] = self._apply_budget(X.loc[neg_mask, [self.head_col]],
#                                             w[neg_mask], self.head_col, self.class_budget_neg)

#         if self.normalize == "per_class":
#             if pos_mask.any(): w[pos_mask] /= max(w[pos_mask].mean(), self.eps)
#             if neg_mask.any(): w[neg_mask] /= max(w[neg_mask].mean(), self.eps)
#         else:
#             w /= max(w.mean(), self.eps)
...
#     def _apply_budget(self, X_sub: pd.DataFrame, w_rows: np.ndarray, entity_col: str,
