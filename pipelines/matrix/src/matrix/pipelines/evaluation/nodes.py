...
def clean_clinical_trial_data(clinical_trial_data: pd.DataFrame, endpoint: str) -> pd.DataFrame:
    ...
    row_has_missing = clinical_trial_data['drug_kg_id'].isna() | clinical_trial_data['disease_kg_id'].isna()
    clinical_trial_data = clinical_trial_data[~row_has_missing].reset_index(drop=True)
    ...
    return clinical_trial_data[['drug_name', 'disease_name', 'drug_kg_id', 'disease_kg_id', 'significantly_better', 'non_significantly_better', 'non_significantly_worse', 'significantly_worse']]
...
def generate_time_split_validation_barplot(
    ...
    cat_name_lst = ['significantly_better', 'non_significantly_better', 'non_significantly_worse']
    ...
    for n, score_type in enumerate(score_type_lst):
        ...
        for m, score_series in enumerate(score_series_lst):
            ax[n].scatter((m+1)*np.ones(len(score_series)), score_series, color = 'orange', s = 5, alpha = 0.5)
...
def _generate_time_split_validation_classification_auroc(
    ...
    y_true = np.concatenate([np.ones_like(treat_probs_pos), np.zeros_like(treat_probs_neg)])
