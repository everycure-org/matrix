...
def _create_evaluation_fold_pipeline(
    model_name: str, evaluation: str, fold: Union[str, int], matrix_input: str, score_col_name: str
) -> Pipeline:
    """Create pipeline for multiple models, evaluation and fold.
        ...
        [
            ArgoNode(
                ...
                inputs={
                    ...
                    "matrix": f"{matrix_input}.fold_{fold}.{model_name}.model_output.sorted_matrix_predictions@pandas",
                    "generator": f"params:evaluation.{evaluation}.evaluation_options.generator",
                },
                outputs=f"evaluation.{matrix_input}.fold_{fold}.{evaluation}.{model_name}.model_output.pairs",
                name=f"{matrix_input}.create_{evaluation}_evaluation_pairs_fold_{fold}_for_{model_name}",
            ),
            ArgoNode(
                func=partial(nodes.evaluate_test_predictions, score_col_name=score_col_name),
                inputs=[
                    f"evaluation.{matrix_input}.fold_{fold}.{evaluation}.{model_name}.model_output.pairs",
                    f"params:evaluation.{evaluation}.evaluation_options.evaluation",
                ],
                outputs=f"evaluation.{matrix_input}.fold_{fold}.{evaluation}.{model_name}.reporting.result",
                name=f"{matrix_input}.create_{evaluation}_evaluation_fold_{fold}_for_{model_name}",
            ),
        ],
        tags=["argowf.fuse", f"argowf.fuse-group.{matrix_input}.{evaluation}.fold_{fold}.{model_name}"],
    )


def _create_core_stability_pipeline(
    model_name: str, fold_main: str, fold_to_compare: str, evaluation: str, matrix_input: str, score_col_name: str
) -> Pipeline:
    if evaluation != "rank_commonality":
        pipeline_nodes = [
            ArgoNode(
                ...
                inputs=[
                    ...
                    f"{matrix_input}.fold_{fold_main}.{model_name}.model_output.sorted_matrix_predictions@pandas",
                    f"{matrix_input}.fold_{fold_to_compare}.{model_name}.model_output.sorted_matrix_predictions@pandas",
                ],
                outputs=f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.{evaluation}.{model_name}.model_stability_output.pairs@pandas",
                name=f"{matrix_input}.create_{fold_main}_{fold_to_compare}_{evaluation}_evaluation_pairs_for_{model_name}",
            ),
            ArgoNode(
                func=partial(nodes.evaluate_stability_predictions, score_col_name=score_col_name),
                inputs=[
                    f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.{evaluation}.{model_name}.model_stability_output.pairs@pandas",
                    f"params:evaluation.{evaluation}.evaluation_options.stability",
                    f"{matrix_input}.fold_{fold_main}.{model_name}.model_output.sorted_matrix_predictions@pandas",
                    f"{matrix_input}.fold_{fold_to_compare}.{model_name}.model_output.sorted_matrix_predictions@pandas",
                ],
                outputs=f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.{evaluation}.{model_name}.model_stability_output.result",
                name=f"{matrix_input}.calculate_{fold_main}_{fold_to_compare}_{evaluation}_for_{model_name}",
        ...
        pipeline_nodes = [
            ArgoNode(
                ...
                inputs=[
                    f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.stability_ranking.{model_name}.model_stability_output.result",
                    f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.stability_overlap.{model_name}.model_stability_output.result",
                ],
                outputs=f"evaluation.{matrix_input}.fold_{fold_main}.fold_{fold_to_compare}.{evaluation}.{model_name}.model_stability_output.result",
                name=f"{matrix_input}.calculate_{fold_main}_{fold_to_compare}_{evaluation}_for_{model_name}",
                argo_config=ARGO_NODE_MEDIUM_MATRIX_GENERATION,
...
def create_model_pipeline(
    model_name: str, evaluation_names: List[str], n_cross_val_folds: int, matrix_input: str, score_col_name: str
) -> Pipeline:
    ...
    for fold in range(n_cross_val_folds):
        for evaluation in evaluation_names:
            pipelines.append(
                pipeline(
                    _create_evaluation_fold_pipeline(model_name, evaluation, fold, matrix_input, score_col_name),
                    tags=[evaluation],
                )
            )
            ...
            pipeline(
                [
                    ArgoNode(
                        ...
                        outputs=f"evaluation.{matrix_input}.{evaluation}.{model_name}.reporting.result_aggregated",
                        name=f"{matrix_input}.aggregate_{evaluation}_evaluation_results_for_{model_name}",
                        tags=[evaluation],
                    ),
                    ...
                    ArgoNode(
                        ...
                        inputs=[
                            f"evaluation.{matrix_input}.{evaluation}.{model_name}.reporting.result_aggregated",
                            "params:evaluation.reported_aggregations",
                        ],
                        outputs=f"evaluation.{matrix_input}.{evaluation}.{model_name}.reporting.result_aggregated_reduced",
                        name=f"{matrix_input}.reduce_aggregated_{evaluation}_evaluation_results_for_{model_name}",
                        tags=[evaluation],
                    ),
                ]
            )
    ...
    for model in settings.DYNAMIC_PIPELINES_MAPPING().get("modelling"):
        ...
        pipelines.append(
            create_model_pipeline(
                model_name, evaluation_names, n_cross_val_folds, matrix_input, score_col_name=score_col_name
            )
        )
                ...
                [
                    ArgoNode(
                        ...
                        inputs={
                            ...
                            **{
                                f"{evaluation}.fold_{fold}": f"evaluation.{matrix_input}.fold_{fold}.{evaluation}.{model_name}.reporting.result"
                                for evaluation in evaluation_names
                                for fold in range(n_cross_val_folds)
                            },
                            # Consolidate aggregated reports per model
                            **{
                                f"{evaluation}.aggregated": f"evaluation.{matrix_input}.{evaluation}.{model_name}.reporting.result_aggregated"
                                for evaluation in evaluation_names
                            },
                        },
