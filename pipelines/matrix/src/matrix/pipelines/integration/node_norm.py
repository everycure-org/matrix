...
class NodeNormalizer:
    def __init__(
        self,
        robokop_nodes: pathlib.Path,
        robokop_edges: pathlib.Path,
        rtx_kg2_nodes: pathlib.Path,
        rtx_kg2_edges: pathlib.Path,
    ):
        load_dotenv()
        ...
        self.nn_endpoint = (
            f'{os.getenv("NODE_NORMALIZER_SERVER", "https://nodenormalization-sri.renci.org/")}/get_normalized_nodes'
        )
    ...
    def __call__(self):
        ...
        joined_edges = robokop_edges_df.merge(
            rtx_kg2_edges_df,
            on=["subject", "predicate", "object", "primary_knowledge_source"],
            how="outer",
            suffixes=("_robokop", "_rtx"),
        )
        ...
        joined_nodes = robokop_nodes_df.merge(
            rtx_kg2_nodes_df, on=["id", "category"], how="outer", suffixes=("_robokop", "_rtx")
        )
