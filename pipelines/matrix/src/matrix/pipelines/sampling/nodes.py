...
def sample_kg_from_ids(
    sampled_nodes: DataFrame, nodes: DataFrame, edges: DataFrame, params: Dict[str, Any]
    ...
    nodes_sampled = nodes.join(sampled_nodes, on="id", how="inner").persist()
    logger.info(f"Nodes found in KG: {nodes_sampled.count()}")

    # Get edges connected to our sampled nodes
    edges_sampled = edges.join(
        sampled_nodes, on=(edges.subject == sampled_nodes.id) | (edges.object == sampled_nodes.id), how="inner"
    ).persist()
