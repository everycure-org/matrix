...
class MlflowHook:
    ...
    def after_context_created(
        ...
        if not OmegaConf.has_resolver("km.random_name"):
            OmegaConf.register_new_resolver("km.random_name", resolve_random_name, use_cache=True)
        ...
        if mlflow.active_run():
            ...
            mlflow_config.server._mlflow_client = MlflowClient(tracking_uri=mlflow_config.server.mlflow_tracking_uri)
            ...
            if conf_mlflow_yml.get("tracking", {}).get("experiment", {}).get("name") is None:
    ...
    def after_catalog_created(
        ...
        for name, dataset in catalog._datasets.items():
            if isinstance(dataset, MlflowMetricsHistoryDataset) and dataset._prefix is None:
                if dataset._run_id is not None:
                    catalog._datasets[name] = MlflowMetricsHistoryDataset(run_id=dataset._run_id, prefix=name)
                else:
    ...
    def before_pipeline_run(self, run_params: Dict[str, Any], pipeline: Pipeline, catalog: DataCatalog) -> None:
        ...
        """
        self._is_mlflow_enabled = _assert_mlflow_enabled(run_params["pipeline_name"], self.mlflow_config)
        ...
        if self._is_mlflow_enabled:
            ...
            self.long_params_strategy = self.mlflow_config.tracking.params.long_params_strategy

            run_name = self.mlflow_config.tracking.run.name or run_params["pipeline_name"] or "__default__"
            ...
            if self._already_active_mlflow:
                ...
                self._logger.info(f"Mlflow run '{mlflow.active_run().info.run_id}' has started")
    ...
    def before_node_run(self, node: Node, catalog: DataCatalog, inputs: Dict[str, Any], is_async: bool) -> None:
        ...
        if self._is_mlflow_enabled:
            ...
            if self.flatten:
                params_inputs = _flatten_dict(d=params_inputs, recursive=self.recursive, sep=self.sep)

            # sanitize params inputs to avoid mlflow errors
            params_inputs = {self.sanitize_param_name(k): v for k, v in params_inputs.items()}
    ...
    def _log_param(self, name: str, value: Union[Dict, int, bool, str]) -> None:
        ...
        if str_value_length <= MAX_PARAM_VAL_LENGTH:
            ...
            self._logger.warning(f"Parameter '{name}' (value length {str_value_length}) is set as a tag.")
        ...
        if self._is_mlflow_enabled:
            if isinstance(pipeline, PipelineML):
                ...
                with TemporaryDirectory() as tmp_dir:
                    ...
                    artifacts = kedro_pipeline_model.extract_pipeline_artifacts(parameters_saving_folder=Path(tmp_dir))
