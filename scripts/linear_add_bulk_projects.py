...
def simplify_name(name, openai_api_key):
    ...
    response = openai.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a helpful assistant that simplifies project names.",
            },
            {
                "role": "user",
                "content": f"Simplify this project name, keeping the 'Subtask X.X' prefix and maintaining the core meaning, but make it shorter and crisper. By no means should the name be longer than 70 characters: '{name}'",
            },
        ],
    )
...
def add(
    ...
    if team_name not in teams:
        print(
            f"Error: Team '{team_name}' not found. Available teams: {', '.join(teams.keys())}"
        )
        return
        ...
        for row in reader:
            ...
            try:
                result = client.execute(
                    CREATE_PROJECT_MUTATION, variable_values=variables
                )
...
def delete(
    team_name: str,
    linear_api_key: Optional[str] = typer.Option(
        None, envvar="LINEAR_API_KEY", prompt=True
    ),
    confirm: Optional[bool] = typer.Option(
        None,
        "--confirm",
        prompt="Are you sure you want to delete all projects? This action cannot be undone.",
    ),
):
    if not confirm:
    ...
    for project in projects:
        try:
            ...
            delete_result = client.execute(
                DELETE_PROJECT_MUTATION, variable_values=delete_variables
            )
